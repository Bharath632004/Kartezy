...
/// Also supports backup code entry as a fallback.
...
class _MfaVerificationPageState extends ConsumerState<MfaVerificationPage> {
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      ...
      final valid = await mfaNotifier.validate(code);

      if (valid && mounted) {
        // MFA verified - navigate to home/dashboard
        context.go('/home');
      } else if (mounted) {
  ...
  Future<void> _useBackupCode() async {
    ...
    try {
      final mfaNotifier = ref.read(mfaProvider.notifier);
      final valid = await mfaNotifier.useBackupCode(widget.email, code);
            ...
            children: [
              ...
              if (_useBackupCode) ...[
                ...
                SizedBox(
                  ...
                  height: 50,
                  child: ElevatedButton(
                    onPressed: _isLoading ? null : _useBackupCode,
                    style: ElevatedButton.styleFrom(
