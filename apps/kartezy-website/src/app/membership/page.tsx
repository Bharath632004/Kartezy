...
const MembershipPage = () => {
  ...
  const loadMembership = useCallback(async () => {
    try {
      ...
      if (apiError.response?.status === 401) {
        ...
        setError(apiError.response?.data?.message || 'Failed to load membership');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleUpgrade = async () => {
    try {
      ...
      setSnackbar({ open: true, message: apiError.response?.data?.message || 'Upgrade failed', severity: 'error' });
    } finally {
      setUpgrading(false);
    }
  };
  ...
  return (
    <Container maxWidth="xl" sx={{ py: { xs: 4, md: 6 } }}>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h2" sx={{ fontWeight: 600, mb: 2 }}>Kartezy Membership</Typography>
      ...
      <Card sx={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', p: { xs: 3, md: 6 }, mb: 4 }}>
        <CardContent>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography variant="h3" sx={{ fontWeight: 600, mb: 1 }}>{membership.level} Member</Typography>
          ...
          {(membership.benefits?.length ?? 0) > 0 && (
            <Box sx={{ mb: 4 }}>
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 2 }}>Membership Benefits</Typography>
          ...
          {membership.points !== undefined && (
            <Box sx={{ mb: 4 }}>
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 2 }}>Loyalty Points</Typography>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                <StarBorder sx={{ fontSize: 40, color: 'primary.main' }} />
                <Box sx={{ flex: 1 }}>
                  <Typography variant="h4" sx={{ fontWeight: 600 }}>{membership.points.toLocaleString()} Points</Typography>
          ...
          {(membership.recentActivity?.length ?? 0) > 0 && (
            <Box sx={{ mb: 4 }}>
              ...
              <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 4, overflow: 'hidden' }}>
                {membership.recentActivity?.map((activity, idx) => (
                  <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    px: 3, py: 2,                    borderBottom: idx < (membership.recentActivity?.length ?? 0) - 1 ? '1px solid #f0f0f0' : 'none' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      {activity.type === 'earned' ? <StarBorder color="primary" /> : <LocalOffer color="primary" />}
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>{activity.description}</Typography>
                        <Typography variant="caption" color="text.secondary">{activity.date}</Typography>
                      </Box>
                    </Box>
                    <Typography variant="body2" sx={{ fontWeight: 600, color: activity.type === 'earned' ? 'success.main' : 'text.primary' }}>
                      {activity.value}
                    </Typography>
                  </Box>
                ))}
      ...
      <Typography variant="h3" sx={{ fontWeight: 600, mb: 4, textAlign: 'center' }}>Choose Your Plan</Typography>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 4 }} key={i}>
            <Card sx={{ borderRadius: 4, boxShadow: plan.featured ? '0 8px 32px rgba(25,118,210,0.15)' : '0 2px 8px rgba(0,0,0,0.08)',
              ...
              <CardContent sx={{ p: 4, textAlign: 'center' }}>
                ...
                <Typography variant="h4" sx={{ fontWeight: 700, mb: 3, color: `${plan.color}.main` }}>{plan.price}</Typography>
