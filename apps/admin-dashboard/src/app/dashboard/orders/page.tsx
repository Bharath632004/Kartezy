...
const OrdersList = () => {
  ...
  const [openReassignDriverDialog, setOpenReassignDriverDialog] = useState(false);
  const [driverIdForReassignment, setDriverIdForReassignment] = useState<string>('');
  ...
  const handleReassignDriverConfirm = async () => {
    if (selectedOrderId && driverIdForReassignment) {
      try {
        await orderService.reassignDriver(selectedOrderId, driverIdForReassignment);
        setOpenReassignDriverDialog(false);
        setSelectedOrderId(null);
        setDriverIdForReassignment('');
      } catch (err) {
  ...
  return (
    <Box sx={{ py: 3 }}>
                ...
                <TableCell align="right">
                  {!order.driverName ? (
                    ...
                    <Tooltip title="Reassign Driver">
                      <IconButton onClick={() => handleReassignDriverClick(order.id)}>
                        <SwapHorizOutlined />
                      </IconButton>
                    </Tooltip>
                  )}
      ...
      <Dialog open={openReassignDriverDialog} onClose={() => setOpenReassignDriverDialog(false)}>
        <DialogTitle>Reassign Driver</DialogTitle>
        <DialogContent>
          Are you sure you want to reassign the driver for this order?
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOpenReassignDriverDialog(false)}>Cancel</Button>
          <Button onClick={handleReassignDriverConfirm} color="warning">
            Reassign Driver
          </Button>
        </DialogActions>
      </Dialog>
