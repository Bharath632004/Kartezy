...
import { Box, Container, Typography, Card, CardContent, Button, Divider, TextField, CircularProgress, Alert, Stepper, Step, StepLabel } from '@mui/material';
    ...
    <Container maxWidth="md" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      <Card sx={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', p: { xs: 3, md: 5 } }}>
        <CardContent>
          {activeStep === 0 && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 3 }}>Cart Items</Typography>
              {cart.items.map((item: any, idx: number) => (
                <Box key={item.productId || idx} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.5, borderBottom: idx < cart.items.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                  <Box>
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>{item.productName || item.name}</Typography>
                    <Typography variant="caption" color="text.secondary">Qty: {item.quantity} × ₹{item.price || item.unitPrice}</Typography>
                  </Box>
                  <Typography variant="body1" sx={{ fontWeight: 600 }}>₹{((item.price || item.unitPrice) * item.quantity).toLocaleString('en-IN')}</Typography>
                </Box>
              ))}
              <Divider sx={{ my: 2 }} />
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, alignItems: 'flex-end' }}>        <Typography variant="body2" sx={{fontWeight: 500}}>Subtotal: ₹{(cart.subtotal ?? cart.totalAmount).toLocaleString('en-IN')}</Typography>
                {cart.discountAmount > 0 && <Typography variant="body2" color="success.main">Discount: -₹{cart.discountAmount.toLocaleString('en-IN')}</Typography>}        <Typography variant="body2" sx={{fontWeight: 500}}>Delivery: ₹{(cart.deliveryFee ?? 0).toLocaleString('en-IN')}</Typography>
                <Typography variant="h6" sx={{ fontWeight: 600 }}>Total: ₹{cart.totalAmount.toLocaleString('en-IN')}</Typography>
              </Box>
          ...
          {activeStep === 1 && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 3 }}>Delivery Address</Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <TextField fullWidth label="Address" value={deliveryAddress.address} onChange={e => setDeliveryAddress(a => ({ ...a, address: e.target.value }))} multiline rows={2} />
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <TextField fullWidth label="City" value={deliveryAddress.city} onChange={e => setDeliveryAddress(a => ({ ...a, city: e.target.value }))} />
                  <TextField fullWidth label="Pincode" value={deliveryAddress.pincode} onChange={e => setDeliveryAddress(a => ({ ...a, pincode: e.target.value }))} />
                </Box>
                <TextField fullWidth label="Phone" value={deliveryAddress.phone} onChange={e => setDeliveryAddress(a => ({ ...a, phone: e.target.value }))} />
              </Box>
          ...
          {activeStep === 2 && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 3 }}>Payment Method</Typography>
          ...
          {activeStep === 3 && (
            <Box sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 600, mb: 1 }}>Order Summary</Typography>
